...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       $ git clone git@github.com:VNG-Realisatie/gemma-autorisatiecomponent
...
``src/ac/conf/local.py``. You can base this file on the
...
1. Clone or download the code from `Github`_ in a folder like
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:VNG-Realisatie/gemma-autorisatiecomponent ac
       Cloning into 'ac'...
       ...
...
environment variables. See ``src/ac/conf/docker.py`` for
all settings.
...
Installation of ``vng-api-common`` makes the binary ``generate_schema`` available,
